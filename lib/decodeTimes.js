...
function decode(encoded_data) {
    ...
    for (i = -1, w = 0, h = 0, prev_t = 0; ++i < i_max;) {
        // atkoduojam reisu pradzios laikus
        var t = times[i];

        if (t == "") {
            //pasibaige reisu pradzios
            break;
        }
    ...
    var final_data = { workdays: weekdays, times: timetable, tag: zero_ground.join(""), valid_from: valid_from, valid_to: valid_to };
